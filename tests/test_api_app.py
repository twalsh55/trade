...
def test_crm_calendar_connect_event_ingest_and_delete_endpoints_manage_meeting_memory() -> None:
    ...
    assert "suggested_action_focus" in overview_response.json()["ambient_memory_summary"]
    assert "suggested_action_kind" in overview_response.json()["ambient_memory_summary"]
    assert "suggested_action_note" in overview_response.json()["ambient_memory_summary"]
    assert "warm_source_labels" in overview_response.json()["ambient_memory_summary"]
    assert "quiet_source_labels" in overview_response.json()["ambient_memory_summary"]
    ...
    assert any(
        label.startswith("Google Calendar")
        for label in overview_response.json()["ambient_memory_summary"]["warm_source_labels"]
    )
