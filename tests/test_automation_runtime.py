...
def test_mailbox_sync_job_reports_counts_and_failure(monkeypatch) -> None:
    monkeypatch.setattr("src.adapters.automation.runtime.run_scheduled_mailbox_sync_job", lambda: (2, 5, 2, 1, 1))
    result = _run_mailbox_sync_job()
    assert result.status == "ok"
    assert result.detail == "connections=2 threads=5 watch_ready=2 watch_renewed=1 event_ready=1"
